...
3. Check you're on the correct branch from PRD `branchName`. If not, check it out or create from current checked out branch.
...
Replace [optional share] with [Session: https://opncd.ai/s/[share-id]] if [share-id] is set.
Remove [optional share] if [share-id] is session is not shared.
...
- Commit frequently using skill git-commit-message-writer
- Keep CI green
